...
class Q_Attention(nn.Module):
    ...
    def __init__(self, config, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        #self.qkv = QuantizeLinear(config.hidden_size, config.hidden_size * 3, clip_val=config.clip_init_val,
                                #  weight_bits=config.weight_bits, 
                                 # weight_layerwise=config.weight_layerwise, 
                                 # weight_quant_method=config.weight_quant_method,
                                 # learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
...
class Q_Block(nn.Module):
    ...
    def __init__(self, config, mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=gelu, norm_layer=Q_OVitLayerNorm):
        super().__init__()
        self.norm1 = norm_layer(config.hidden_size)
        self.attn = Q_Attention(config=config, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
class Q_PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64,norm_layer=nn.BatchNorm2d,use_norm=True):
        super().__init__()
        ...
        self.norm = norm_layer(
            embed_dim) if use_norm else nn.Identity()
...
class lowbit_VisionTransformer(nn.Module):
    ...
    def __init__(self, config, patch_size=4, in_chans=3, num_classes=1000, depth=12,
        ...
        self.patch_embed = embed_layer(img_size=config.input_size, patch_size=patch_size, in_chans=in_chans, embed_dim=config.hidden_size)
...
def onebits_deit_small_patch16_224_w32(config,num_classes, **kwargs):
    ...
    return model
def onebits_deit_small_patch16_224_2(config,num_classes, **kwargs):
    model = lowbit_VisionTransformer(config=config, num_classes=num_classes,patch_size=16, depth=7, mlp_ratio=4, qkv_bias=True,
                                     norm_layer=Q_OVitLayerNorm, **kwargs)
    model.default_cfg = _cfg()
